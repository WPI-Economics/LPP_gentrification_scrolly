...
format: 
  closeread-html:
    ...
    fontsize: "20pt"
    cr-style:
      narrative-background-color-sidebar: "#E2F0F2" #e6f2f4 is the light blue on the LPP website - just fyi
...
Introduction:@cr-intro-title

[@cr-intro-title]{highlight="cr-xx,cr-yy,cr-zz,cr-aa,cr-bb,cr-cc"}


:::{#cr-intro-title}
  
| At the time of this analysis, for two years in a row, London’s [poverty rate had fallen]{#cr-xx} to the lowest on record. <br><br>Yet at the same time other data in the London Poverty Profile pointed at [soaring rents]{#cr-yy} and a once in a generation [cost of living crisis]{#cr-zz}. <br><br>We wanted to better understand the mechanisms at work behind this drop in poverty, and wondered if a key factor could be [low-income Londoners being priced out and replaced by wealthier residents]{#cr-aa}. aka [GENTRIFICATION]{#cr-bb}
      
:::

What is gentrification?:@cr-gentrification-cdrc-definition

[@cr-gentrification-cdrc-definition]{highlight="cr-rapidly,cr-changes,cr-its,cr-population,cr-influx,cr-displacement, cr-breakup"}
...
For our analysis:@cr-gentrification-definition
...
| The [influx]{#cr-influx} [of]{#cr-of1} [more]{#cr-more} [affluent]{#cr-affluent} [residents]{#cr-residents} to [lower]{#cr-lower} [income]{#cr-income} areas, relative to the residents that were already there leading to the [displacement]{#cr-displacement} [of]{#cr-of2} [the]{#cr-the} [previous]{#cr-previous} [population]{#cr-population}. 

:::


Data sources used:@cr-measurement


:::{#cr-measurement}

<b>Identification of "gentrified" areas</b>

-   Changes in income: Income estimates for small areas, 2010/12 - 2018/20, Middle Super Output Area level (MSOA), CPIH adjusted into 2020 prices (ONS)

<b>Core Analysis</b> 

-   Population changes: Census 2021 compared to Census 2011 (ONS)

<b>Further exploration/validation</b>

-   House price changes for 2010 to 2023 (HM Land registry)
-   Changes in primary school applications between 2013/14 and 2023/24 (DfE)
...
This boxplot shows median incomes within all MSOAs across London, adjusted to 2020 prices.  @cr-income-boxplots

Looking at each of ONS' small area income estimates since 2011/12 together, we see average MSOA incomes appear fairly stable between 2011/12 and 2019/20.

::: {#cr-income-boxplots}
```{r income-boxplots, include = TRUE, echo = FALSE, message = FALSE, warning = FALSE}

library(tidyverse)
library(highcharter)

source("LPP colours.r")


TITLE <- "MSOA income estimates (2011/12 - 2019/20, in 2020 prices)"
Title_csv <- ""
Note <- ""
Data_Source <- ""
...
df <- read_csv("plot-inputs/1_small-area-incomes.csv") %>% 
  ...
  rename("2011/12" = `2012`,
         ...
         "2017/18" = `2018`,
         "2019/20" = `2020`) %>% 
  pivot_longer(cols = c(`2011/12`, `2013/14`, `2015/16`, `2017/18`, `2019/20`),
               names_to = "Year",
               values_to = "Income") %>% 
  data_to_boxplot(., 
...
The MSOA average income distributions are broadly similar across years. Starting with 2011/12, @cr-income-density-plots-2012
...
TITLE <- "Distribution of MSOA incomes between 2011/12 and 2019/20"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
then 2013/14, @cr-income-density-plots-2014
...
2015/16, @cr-income-density-plots-2016
...
2017/18, @cr-income-density-plots-2018
...
and 2019/20. @cr-income-density-plots-2020
...
However, if we look at 2011/12 and 2019/20 only, we see that there are some differences in the distribution of neighbourhood incomes. @cr-income-density-plots-off 

Compared to 2011/12, there are more incomes in 2019/20 that centre towards the middle of the distribution. [@cr-income-density-plots-off]{pan-to="40%,40%" scale-by="2.0"}

and fewer incomes at the higher end of the distribution. [@cr-income-density-plots-off]{pan-to="-55%,-30%" scale-by="2.0"}

This means that, although median neighbourhood incomes are not very different between the two years, there are fewer neighbourhoods with very high average incomes in 2019/20, compared to 2011/12. @cr-income-density-plots-off
...
In 2011/12, the lowest income quintile had an average income of just under £30k @cr-income-byQuin-medians-2012-withHighlightLowest
...
TITLE <- "Median 2011/12 MSOA incomes"
SUBTITLE <- "By area income quintile in 2011/12"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
By 2019/20, incomes had only slightly increased. @cr-income-byQuin-medians-2020

The 60% of MSOAs with the lowest incomes in 2011/12 had slightly higher median incomes in 2019/20. @cr-income-byQuin-medians-2020-withHighlightLowest

The highest income MSOAs in 2011/12 actually saw a drop in their median incomes in 2019/20. @cr-income-byQuin-medians-2020-withHighlightHighest
...
TITLE <- "Median MSOA incomes in 2011/12 and 2019/20, by area income quintile in 2011/12"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
#TITLE <- "Median MSOA incomes, by area income quintile in 2011/12"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
Here we have calculated the percentage difference between the 2011/12 and 2019/20 incomes, still split by the 2011/12 quintile groups @cr-income-change-density

If we take a look at the distribution of those percentage changes in income, we see that there are interesting differences between the 2011/12 quintiles. 

Specifically, when looking at quintiles 1 and 5, we see that the percentage changes in income between 2011/12 and 2019/20 are a lot higher for areas in the lowest income quintile in 2011/12 compared to areas in the highest income quintile in that year. 

A lot of this around the middle of the distributions could be to do with increases to minimum/living wage rates @cr-income-change-density-1st5thQuin

But quintile 1 also has a longer right tail, indicating very high average income increases in a small number of places. [@cr-income-change-density-1st5thQuin]{pan-to="-55%,-40%" scale-by="1.5"}
...
TITLE <- "Distribution of changes in MSOA incomes between 2011/12 and 2019/20"
SUBTITLE <- "By area income quintile in 2011/12"
Title_csv <- ""
Note <- ""
Data_Source <- ""
...
plot <- highchart() %>% 
  ...
  hc_xAxis(title = list(text = "Percentage change in MSOA incomes between 2011/12 - 2019/20")) %>%
  
  hc_yAxis(title = list(text = "Density")) %>%
  
  hc_add_series(type = "area",
                name = "1st income quintile in 2011/12 (lowest)",
                data = density(df$pc_change_2012_2020[df$income_quintiles_2012 == 1]),
                color = discreet.set[[1]]) %>%
  
  hc_add_series(type = "area",
                name = "2nd income quintile in 2011/12",
                data = density(df$pc_change_2012_2020[df$income_quintiles_2012 == 2]),
                color = discreet.set[[2]]) %>%
  
  hc_add_series(type = "area",
                name = "3rd income quintile in 2011/12",
                data = density(df$pc_change_2012_2020[df$income_quintiles_2012 == 3]),
                color = discreet.set[[3]]) %>%
  
  hc_add_series(type = "area",
                name = "4th income quintile in 2011/12",
                data = density(df$pc_change_2012_2020[df$income_quintiles_2012 == 4]),
                color = discreet.set[[4]]) %>%
  
  hc_add_series(type = "area",
                name = "5th income quintile in 2011/12 (highest)",
...
Here are incomes in 2011/12 and 2019/20 plotted. @cr-incomes-scatterplot

For the rest of the analysis we are going to concentrate on the areas belonging to quintile 1 in 2011/12. [@cr-incomes-scatterplot]{pan-to="55%,-30%" scale-by="1.75"}
...
TITLE <- "Comparison of small area incomes, 2011/12 and 2019/20"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
plot <- highchart() %>% 
  ...
  hc_yAxis(title = list(text = "Incomes in 2019/20")) %>%
  
  hc_xAxis(title = list(text = "Incomes in 2011/12")) %>%
...
This plot shows area incomes in 2011/12 and 2019/20 for those areas that were in the lowest income quintile in 2011/12, with the median incomes for each year annotated. We identified areas of interest to us as those that were below the median in 2011/12 and above the median in 2019/20. @cr-incomes-scatterplot-1stQuin
...
TITLE <- "Comparison of small area incomes (2011/12 and 2019/20), for areas with the lowest 20% of incomes in 2011/12"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
df <- read_csv("plot-inputs/1_small-area-incomes.csv") %>% 
  ...
  group_by(income_quintiles_2012) %>% 
  mutate(
  change_grid = case_when(
    `2012` > median(`2012`) & `2020` > median(`2020`) ~ 1,
    `2012` > median(`2012`) & `2020` < median(`2020`) ~ 2,
    `2012` <= median(`2012`) & `2020` >= median(`2020`) ~ 3,
    `2012` < median(`2012`) & `2020` < median(`2020`) ~ 4
  )) %>% ungroup() %>% 
  mutate(change_grid = case_when(income_quintiles_2012 == 1 ~ change_grid,
                                 TRUE ~ NA))
  ...
  hc_yAxis(title = list(text = "Incomes in 2019/20"),
           plotLines = list(
             list(color = discreet.set[6],
                  ...
                  label = list(text = paste0("Median income in 2019/20: £", 
  ...
  hc_xAxis(title = list(text = "Incomes in 2011/12"),
           plotLines = list(
             list(color = discreet.set[2],
                  ...
                  label = list(text = paste0("Median income in 2011/12: £", 
...
And here they are on a map, coloured according to the average income change quadrant between 2011/12 and 2019/20. @cr-incomes-map-1stQuin

The neighbourhoods in the Low to High quadrant in deep pink we will subsequently call the "Highest income rises".
...
TITLE <- "Map of changes in MSOA incomes between 2011/12 and 2019/20"
SUBTITLE <- "Lowest income quintile in 2011/12 only"
Title_csv <- ""
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
dfx <- read_csv("plot-inputs/1_small-area-incomes.csv") %>% 
  select("MSOA name" = MSOA21NM, MSOA21HCLNM, pc_change_2012_2020, income_quintiles_2012, `2012`, `2020`)

#pivot the quintiles into series
# dfx <- dfx %>% 
#   mutate(income_quintiles_2012 = case_when(
#     income_quintiles_2012 == 1 ~ "1st income quintile in 2011/12 (lowest)",
#     income_quintiles_2012 == 2 ~ "2nd income quintile in 2011/12",
#     income_quintiles_2012 == 3 ~ "3rd income quintile in 2011/12",
#     income_quintiles_2012 == 4 ~ "4th income quintile in 2011/12",
#     income_quintiles_2012 == 5 ~ "5th income quintile in 2011/12 (highest)",
#     TRUE ~ "Unknown"
#   )) 
...
dfx <- dfx %>% 
  mutate(pc_change_2012_2020 = case_when(
    `income_quintiles_2012` == 1 ~ pc_change_2012_2020,
    TRUE ~ NA
  ),
  change_ntile = ntile(pc_change_2012_2020, 5)) %>% 
  group_by(income_quintiles_2012) %>% 
  mutate(
  change_grid = case_when(
    `2012` > median(`2012`) & `2020` > median(`2020`) ~ 1,
...
# msoa_bounds <- s3read_using(readRDS, # here you tell the s3read_using which read function you are using, and the arguments of the function can follow inside after a comma as you would in the original function)
#                             object = "msoa21_bounds_UGC.rds", 
#                             bucket = "wpi-spatial-data/MSOA")

msoa_bounds <- readRDS("msoa21_bounds_UGC.rds")
...
highchart() %>%
  hc_add_series_map(map = msoa_bounds,
                    ...
                    borderColor = "black",
                    borderWidth = 0.3,
                    name = "Change in mean income 2011/12 - 2019/20",
                    value = "change_grid", #"pc_change_2012_2020" , #this is the column in the df to be mapped
  ...
  hc_colorAxis(
    dataClassColor = "change_grid",
    #this adds categorical data to the map and legend
    dataClasses = list(
      list(name = "low/high (biggest rises)", from = 3, to = 3, color = discreet.set[6]),
      list(name = "high/high", from = 1, to = 1, color = red.5[5]),
      list(name = "high/low", from = 2, to = 2, color = green.3[2]),
      list(name = "low/low", from = 4, to = 4, color = green.3[3]))
  ) %>% 
  ...
  hc_tooltip(useHTML = T,
             ...
             pointFormat = "<b>{series.name}</b><br>
              ...
              <b>Change: </b>{point.pc_change_2012_2020}%<br>"
  ) %>% 
...
And here are the 'biggest rises' areas tallied by borough @cr-tally

:::{#cr-tally}

```{r tally, include = TRUE, echo = FALSE, message = FALSE}

#tally chart

#map of the msoas that were in the low income quyn and have seen most change
...
TITLE <- "Number of gentrified areas by London Borough"
Title_csv <- ""
Note <- "Neighbourhoods that were in the lowest income quintile in 2012 and have seen above average change in mean income between 2012 and 2020"
Data_Source <- "Income estimates for small areas, ONS, 2011/12 and 2019/20"
Chart_Source <- "London's Poverty Profile 2025"


df <- read_csv("plot-inputs/6_tally.csv") %>% 
  arrange(desc(tally))
...
plot <- highchart() %>% 
  ...
  hc_yAxis(title = list(text = "Number of neighbourhoods"),
           max = 12) %>%
  
  hc_xAxis(categories = df$LADNM, 
           title = list(text = "")) %>%
  
  hc_add_series(type = "column",
                name = "Gentrified areas",
                data = df$tally,
                color = green.pair[1],
                showInLegend = F) %>%
...
Using census data inside these groups to examine the population differences as closely matched temporally to 2011/12 and 2019/20 as we can we see:
...
Title_csv <- "Highest income areas are those that were below the median income for London in 201/12 and above the median in 2019/20."
Note <- ""
Data_Source <- ""
Chart_Source <- "London's Poverty Profile 2024"
...
One of the most remarkable drops is in children under 16 and there are also fewer young (16-29) and older (70+) people living in the areas with the biggest income increases. On the other hand, there are more people between 30 and 69 years living in those areas. @cr-census-age
...
Worth noting here that primary school applications for 2023/24 have dropped in the whole of England, compared to 10 years ago. @cr-school-applications-ldn

This is particularly stark in central London - and these `gentrified` neighbourhoods appear to be contributing to this trend.
...
bounds <- readRDS("LAUA23_BGC_simple2024-12-16.rds")
...
There is a also a drop in black people in those areas not seen in the other London categories. @cr-census-ethnicity

White people are also dropping, but at a slightly lower level than the other London categories.

Other ethnic, Asian, and mixed or multiple ethnic backgrounds saw increases between censuses.
...
Increase in couples without children - higher than in the other London neighbourhoods. @cr-census-hhcomp
...
House prices in those areas were still lower than the rest of London but appear to have broken away from other areas in quintile 1 that did not see such big income increases. @cr-house-prices
...
When indexed to start at 2010 (=100), we see house prices in those areas increased at a much higher rate than the rest of London and the rest of the areas in quintile 1. @cr-house-prices-index
...
In summary we found: @cr-questions1

::: {#cr-questions1}

- A small number of neighbourhoods in London that had seen very large changes in average incomes 
- These areas were most commonly found in East London (Tower Hamlets, Newham), but also Brent, Southwark, and Haringey.
- Changes in the housing stock of these places to the detriment of social housing in favour of private rental
- Demographics shifts with decreasing proportions of children, black people
- Increasing couples without children, Asian, and those from 'other' ethnic backgrounds

:::

Questions left to answer! @cr-questions2

::: {#cr-questions2}

- We see the composition of these areas changing, but are people being displaced?
- If they are, where are they going?
- And ultimately what does all of this mean for the future of London?
